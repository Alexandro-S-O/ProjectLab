...
void searchHistoryByTime(char startTime[30], char endTime[30]) {
    FILE *fp = fopen("history.txt", "r");
    if (fp == NULL){
        printf("File isn't exist\n");
        return;
        ...
        int match = 0;
        while (fgets(result, sizeof(result), fp) != NULL){
            timeInHistory = strtok(result, "]"); 
            timeInHistory++; 
            if (strcmp(timeInHistory, startTime) >= 0 && strcmp(timeInHistory, endTime) <= 0){
                printf("%s", result);
                match = 1;
            }
        }
        if (!match){
            printf("History tidak ditemukan pada jangka waktu ini.\n");
        }
    }
    fclose(fp);
}
...
int main (){
	...
	if (menu == 1){
        ...
        printf("Masukkan waktu mulai (YYYY-MM-DD HH:MM:SS): ");
        scanf(" %[^\n]", startTime); 
        getchar();
        printf("Masukkan waktu akhir (YYYY-MM-DD HH:MM:SS): ");
        scanf(" %[^\n]", endTime);
        getchar();
